...
if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
	...
	if ($module=='download' AND $act=='hapus'){
		...
		$str_cari = mysql_query("SELECT * FROM download WHERE id_download='$_GET[id]'");

		$r_cari = mysql_fetch_array($str_cari);

		unlink("../../files/$r_cari[nama_file]");   
	...
	if ($file_extension=='php'){
		...
		echo "<script>window.alert('Upload Gagal, Pastikan File yang di Upload tidak bertipe *.PHP');window.location=('../module.php?module=download')</script>";
		...
		mysql_query("INSERT INTO download(judul, nama_file, tgl_posting) 

                            VALUES('$_POST[judul]', '$nama_file', '$tgl_sekarang')");
...
//--------------------------------------------------------------// Input download
	    ...
	    mysql_query("INSERT INTO download(judul, tgl_posting) VALUES('$_POST[judul]', '$tgl_sekarang')");
	...
	if (empty($lokasi_file)){
		...
		if ($file_extension=='php'){
			...
			echo "<script>window.alert('Upload Gagal, Pastikan File yang di Upload tidak bertipe *.PHP');
						  ...
						  window.location=('../module.php?module=download')</script>";
			...
			mysql_query("UPDATE download SET judul = '$_POST[judul]', nama_file = '$nama_file' WHERE id_download = '$_POST[id]'");
