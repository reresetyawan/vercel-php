...
<div class='box'>
	...
	<div><input type=button value='Tambah Berita' onclick=\"window.location.href='?module=berita&act=tambahberita';\"></div>

	<div class='right'>

		<form method=get action='$_SERVER[PHP_SELF]'>

			<input type=hidden name=module value=berita>Masukkan Judul Berita : <input type=text name='kata'> <input type=submit value=Cari>
...
<tr><th>no</th>
	...
	<th>aksi</th>
				...
				if ($_SESSION[leveluser]=='admin'){
					...
					$tampil=mysql_query("SELECT * FROM berita WHERE username='$_SESSION[namauser]' ORDER BY id_berita DESC LIMIT $posisi,$batas");
...
<tr><td align=right width='40'>$no &nbsp;</td>
	...
	<td width='85'>
		...
		<a href=?module=berita&act=editberita&id=$r[id_berita]><img src='images/edit.png' border='0' title='edit' /></a>

		<a href=\"$aksi?module=berita&act=hapus&id=$r[id_berita]&namafile=$r[gambar]\" onClick=\"return confirm('Apakah Anda benar-benar mau menghapusnya?')\">

		<img src='images/del.png' border='0' title='hapus' /></a></td>
			...
			}else{ echo "<table class='list'><thead>  
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td align=right>$no &nbsp;</td>
                ...
                <td>$tgl_posting</td>
		            ...
		            <td><a href=?module=berita&act=editberita&id=$r[id_berita]><img src='images/edit.png' border='0' title='edit' /></a>
...
echo "<div class=\"pagination\"> $linkHalaman</div>";
  ...
  case "editberita":
   ...
   if ($r[headline]=='Y'){
      ...
      echo "<tr><td>Isi Berita</td>

	  			<td><textarea id='loko' name='isi_berita' style='width: 800px; height: 350px;'>$r[isi_berita]</textarea></td></tr>
    ...
    $d = GetCheckboxes('tag', 'tag_seo', 'nama_tag', $r[tag]);
			...
			<tr><td colspan=2><input type=submit value=Update> <input type=button value=Batal onclick=self.history.back()></td>
